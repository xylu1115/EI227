		...
		if (uart_receive_status)
		{
			...
			while(UARTCharsAvail(UART0_BASE))    										// Loop while there are characters in the receive FIFO.
			{
				...
				if (uart_receive_char <= 'z' && uart_receive_char >= 'a')
					uart_receive_char -= 'a' - 'A'; //转换为大写
				str[s++] = uart_receive_char; // 接受的字符存至str字符串
			}
			if (strncmp(str, "AT+CLASS", 8) == 0)
			{
				UARTStringPut("CLASSF1803202\r\n");
			}
			else if(strncmp(str, "AT+STUDENTCODE", 14) == 0)
			{
				UARTStringPut("STUDENT518021910416\r\n");
			}
			else
			{
