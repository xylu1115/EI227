...
int main(void)
{
	...
	IntPrioritySet(FAULT_SYSTICK,0x3); 	//设置优先级相同
...
void SysTick_Handler(void)
{
	...
	//任务2：点亮D1，长按SW1点亮D2，释放后熄灭
	GPIOPinWrite(GPIO_PORTN_BASE, GPIO_PIN_1,GPIO_PIN_1);	//点亮PN1（D1）
...
void UART0_Handler(void)
{
	...
	//任务3：熄灭D1,D2，长按SW2保持接收状态
	GPIOPinWrite(GPIO_PORTN_BASE, GPIO_PIN_0|GPIO_PIN_1,0);	//熄灭PN0,PN1
	while(GPIOPinRead(GPIO_PORTJ_BASE,GPIO_PIN_1) == 0) 	//PJ1(USR_SW2) keep pressed
	{		
		while(UARTCharsAvail(UART0_BASE))     								// Loop while there are characters in the receive FIFO.
		{	
			UARTCharPutNonBlocking(UART0_BASE, UARTCharGetNonBlocking(UART0_BASE));	//UART echo
		}
	}
	while(UARTCharsAvail(UART0_BASE))   // in case PJ1(USR_SW2) haven't beed pressed.
	{
