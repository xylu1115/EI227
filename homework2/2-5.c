...
int main(void)
{
	...
	while (1)
	{
		if (state == 1) flashtime = 1000; //间隔1s
		else if (state == 2) flashtime = 2000; // 间隔2s
		else if (state == 3) flashtime = 200; // 间隔0.2s
		else if (state == 4) // 回到上电初始状态，间隔0.5s
		{
			flashtime = 500;
			state = 0;
		}
		if (systick_100ms_status) //100ms定时到
		{
			...
			if (++i2c_flash_cnt	>= flashtime/100)  //5*100ms=500ms
			{
				i2c_flash_cnt = 0;
				cnt = (cnt+1) % 8;
		...
		//LED跑马灯
		result = I2C0_WriteByte(PCA9557_I2CADDR,PCA9557_OUTPUT,~(1<<cnt|1<<(cnt+1)%8));
		//数码管跑马灯
		result = I2C0_WriteByte(TCA6424_I2CADDR,TCA6424_OUTPUT_PORT2,0); //防拖影
		result = I2C0_WriteByte(TCA6424_I2CADDR,TCA6424_OUTPUT_PORT1,seg7[(cnt1+cnt)%8+1]);
		result = I2C0_WriteByte(TCA6424_I2CADDR,TCA6424_OUTPUT_PORT2,1<<(cnt1+cnt)%8);
...
void GPIOJ_Handler(void)
{
	...
	SysCtlDelay( ui32SysClock / 150); // 用于消抖
}